...
static int part_validate_eraseblock(struct mtdids *id, struct part_info *part)
{
	...
	if (!mtd->numeraseregions) {
		 ...
		 */
		reminder = (u64)part->offset & (mtd->erasesize - 1);
		if (reminder) {
		...
		reminder = (u64)part->size & (mtd->erasesize - 1);
		if (reminder) {
...
static void list_partitions(void)
{
	 ...
	 */
	puts("mtdparts: ");
	puts(nand_platform_data.part_default ? nand_platform_data.part_default : "none");
	puts("\n");
}
		       ...
		       char * const argv[])
{
	if (argc == 2) {
		if (strcmp(argv[1], "default") == 0) {
			...
			setenv("mtdparts", (char *)nand_platform_data.part_default);
			setenv("partition", NULL);
