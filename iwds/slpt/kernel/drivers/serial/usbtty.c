 ...
 * Defines USBTTY_BUFFER_SIZE = sizeof((u16)URB_BUF_SIZE) * 2
 */
#define USBTTY_BUFFER_SIZE (URB_BUF_SIZE * 4)
 ...
 * Global flag
 */
volatile int usbtty_configured_flag = 0;
...
int drv_usbtty_init (void)
{
	...
	if(rc)
		serial_printf("usbttydev stdio register error\n");
	return (rc == 0) ? 1 : rc;
}
...
static void usbtty_init_terminal_type(short type)
{
	switch(type){
		...
		default:
			...
			/* FIXME: BUG ? data_endpoints[2] not define !!! */
			ep_descriptor_ptrs[2] =
...
int extty_fill_buffer (void)
{
	if (usbtty_configured ()) {
		return fill_buffer (&usbtty_input);
	}
	return 0;
}
