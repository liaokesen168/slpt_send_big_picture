 ...
 *  M200 SoC NAND controller driver
 *
 *  TODO:
 *
 *  <Performance>
...
#include <asm/arch/m200_nand.h>
#endif
#include <asm/arch/board_special.h>

#define DRVNAME "m200-nand"
...
struct m200_nand_dma {
	m200_dma_chan_t *chan;
	dma_slave_config_t cfg;

	m200_dma_req_type_t type;
};
...
struct m200_nand {
	...
	struct m200_nand_dma dma_pipe_nand;
	...
	struct m200_nand_platform_data *pdata;
};
 ...
 * ******************************************************
 */
int micron_nand_pre_init(struct m200_nand *nand)
{
...
int samsung_nand_pre_init(struct m200_nand *nand)
{
	struct nand_chip *chip = &nand->chip;
	struct mtd_info *mtd = nand->mtd;
	nand_flash_info_t *nand_info = nand->curr_nand_flash_info;
...
static inline struct m200_nand *mtd_to_m200_nand(struct mtd_info *mtd)
{
	return container_of(mtd->priv, struct m200_nand, chip);
}

static inline int m200_nand_chip_is_ready(struct m200_nand *nand,
...
static void m200_nand_enable_wp(nand_flash_if_t *nand_if, int enable)
{
	int low_assert;
	int gpio;
...
static int m200_nand_dev_is_ready(struct mtd_info *mtd)
{
	struct m200_nand *nand;
	nand_flash_if_t *nand_if;

	int ret = 0;

	nand = mtd_to_m200_nand(mtd);
	nand_if = nand->nand_flash_if_table[nand->curr_nand_flash_if];

	if (nand_if->busy_gpio != -1) {
		ret = m200_nand_chip_is_ready(nand, nand_if);
	} else {
...
static void m200_nand_select_chip(struct mtd_info *mtd, int chip)
{
...
static void
m200_nand_cmd_ctrl(struct mtd_info *mtd, int cmd, unsigned int ctrl)
{
	struct m200_nand *nand;
	nand_flash_if_t *nand_if;
...
static void m200_nand_ecc_hwctl(struct mtd_info *mtd, int mode)
{
	/*
...
static inline void
m200_nand_delay_after_command(struct m200_nand *nand,
...
static inline void
m200_nand_delay_after_address(struct m200_nand *nand,
		nand_flash_info_t *nand_info,
		unsigned int command)
{
	switch (command) {
...
static void m200_nand_command(struct mtd_info *mtd, unsigned int command,
			 int column, int page_addr)
{
	...
	m200_nand_delay_after_command(nand, nand_info, command);
	...
	m200_nand_delay_after_address(nand, nand_info, command);
...
static void m200_nand_command_lp(struct mtd_info *mtd,
...
static int m200_nand_dma_nand_filter(m200_dma_chan_t *dma_chan)
{
	/*
...
static int m200_nand_request_dma(struct m200_nand* nand)
{
	nand_flash_if_t *nand_if;
	...
	nand->dma_pipe_nand.chan = m200_dma_alloc_chan(
			m200_nand_dma_nand_filter);
...
static int m200_nand_ecc_calculate_bch(struct mtd_info *mtd,
...
static int m200_nand_ecc_correct_bch(struct mtd_info *mtd, uint8_t *dat,
...
m200_nand_match_nand_chip_info(struct m200_nand *nand)
{
	struct mtd_info *mtd;
	struct nand_chip *chip;
...
static inline void m200_nand_cpu_read_buf(struct mtd_info *mtd,
		uint8_t *buf, int len)
{
	int i;
...
static inline void m200_nand_noirq_dma_read(struct mtd_info *mtd,
		uint8_t *addr, int len)
{
	struct m200_nand *nand = mtd_to_m200_nand(mtd);
...
static void m200_nand_read_buf(struct mtd_info *mtd, uint8_t *buf, int len)
{
	if (((uint32_t)buf & CONFIG_SYS_CACHELINE_SIZE)
			|| (len & CONFIG_SYS_CACHELINE_SIZE))
		m200_nand_cpu_read_buf(mtd, buf, len);
	else
		m200_nand_noirq_dma_read(mtd, buf, len);
}

static inline void m200_nand_cpu_write_buf(struct mtd_info *mtd,
...
static inline void m200_nand_noirq_dma_write(struct mtd_info *mtd,
...
static void m200_nand_write_buf(struct mtd_info *mtd,
		const uint8_t *buf, int len)
{
	if (((uint32_t)buf & CONFIG_SYS_CACHELINE_SIZE)
			|| (len & CONFIG_SYS_CACHELINE_SIZE))
		m200_nand_cpu_write_buf(mtd, buf, len);
	else
		m200_nand_noirq_dma_write(mtd, buf, len);
}

static int m200_nand_pre_init(struct m200_nand *nand)
{
...
static int m200_nand_onfi_set_features(struct mtd_info *mtd,
...
static int m200_nand_onfi_get_features(struct mtd_info *mtd,
		struct nand_chip *chip, int addr, uint8_t *subfeature_param)
{
	if (!chip->onfi_version)
		return -EINVAL;
...
static int m200_write_spl_reg(struct mtd_info *mtd, loff_t to,
...
static int m200_read_spl_reg(struct mtd_info *mtd, loff_t from,
			size_t len, size_t *retlen, u_char *buf)
{
	int i, j, pages, ret = 0;
	int eccsize, eccbytes, eccsteps;
...
static int m200_get_spl_info(struct mtd_info *mtd,
...
static int m200_nand_init(int devnum)
{
	int ret = 0;
	int bank = 0;
	int i = 0, j = 0, k = 0, m = 0;
	...
	struct m200_nand *nand;
	struct m200_nand_platform_data *pdata = &nand_platform_data;
	nand_flash_if_t *nand_if;

	nand = kzalloc(sizeof(struct m200_nand), GFP_KERNEL);
	if (!nand) {
		printk(DRVNAME
			": Failed to allocate m200_nand.\n");
		return -ENOMEM;
	}
	...
	chip->cmdfunc     = m200_nand_command;
	chip->dev_ready   = m200_nand_dev_is_ready;
	chip->select_chip = m200_nand_select_chip;
	chip->cmd_ctrl    = m200_nand_cmd_ctrl;
	chip->onfi_get_features = m200_nand_onfi_get_features;
	chip->onfi_set_features = m200_nand_onfi_set_features;
	...
	case NAND_XFER_DMA_POLL:
		 ...
		 * DMA transfer
		 */
		ret = m200_nand_request_dma(nand);
		if (ret) {
			printk(DRVNAME" Failed to request DMA channel.\n");
			goto err_free_wp_gpio;
		}

		chip->read_buf  = m200_nand_read_buf;
		chip->write_buf = m200_nand_write_buf;
	...
	case NAND_XFER_CPU_POLL:
		 ...
		 * CPU transfer
		 */
		chip->read_buf  = m200_nand_cpu_read_buf;
		chip->write_buf = m200_nand_cpu_write_buf;
	...
	nand->curr_nand_flash_info = m200_nand_match_nand_chip_info(nand);
	...
	if (mtd->writesize > 512)
		chip->cmdfunc = m200_nand_command_lp;
	...
	case NAND_ECC_TYPE_HW:
		...
		chip->ecc.calculate = m200_nand_ecc_calculate_bch;
		chip->ecc.correct   = m200_nand_ecc_correct_bch;
		chip->ecc.hwctl     = m200_nand_ecc_hwctl;
		chip->ecc.size  =
			nand->curr_nand_flash_info->ecc_step.data_size;
	...
	mtd->_write_user_prot_reg = m200_write_spl_reg;
	mtd->_read_user_prot_reg = m200_read_spl_reg;
	mtd->_get_user_prot_info = m200_get_spl_info;

	printk(DRVNAME
		": Successfully registered m200 SoC NAND controller driver.\n");
...
err_dma_release_channel:
	if (nand->xfer_type == NAND_XFER_DMA_POLL)
		m200_dma_release_chan(nand->dma_pipe_nand.chan);
...
static int
m200_nand_debugfs_show(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	struct m200_nand *nand = mtd_to_m200_nand(&nand_info[0]);
	nand_flash_if_t *nand_if;
	nand_flash_info_t *nand_info = nand->curr_nand_flash_info;
	int i, j;
...
U_BOOT_CMD(
	nandinfo, 1, 1, m200_nand_debugfs_show,
	"show NAND info",
	"cmd: nandinfo"
);
 ...
 * MODULE_DESCRIPTION("NAND controller driver for M200 SoC");
