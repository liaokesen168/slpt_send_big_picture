...
struct usb_interface_descriptor *usbd_device_interface_descriptor (
		...
		int configuration, int interface, int alternate)
{
	struct usb_interface_instance *interface_instance;
...
struct usb_endpoint_descriptor *usbd_device_endpoint_descriptor_index (
		struct usb_device_instance *device, int port,
		int configuration, int interface, int alternate, int index)
{
 ...
 */
int usbd_device_endpoint_transfersize (struct usb_device_instance *device,
		int port, int configuration, int interface, int alternate, int index)
{
	struct usb_alternate_instance *alternate_instance;

	if (!(alternate_instance = usbd_device_alternate_instance (device, port,
...
struct usb_endpoint_descriptor *usbd_device_endpoint_descriptor (
		struct usb_device_instance *device, int port,
		int configuration, int interface, int alternate, int endpoint)
{
			    ...
			    struct usb_endpoint_instance *endpoint)
{
	...
	if (!(urb = (struct urb *) memalign (CONFIG_SYS_CACHELINE_SIZE,
					sizeof (struct urb)))) {
		usberr (" F A T A L:  malloc(%zu) FAILED!!!!", sizeof (struct urb));
		return NULL;
	}
		...
		usb_device_event_t event, int data)
{
	...
	default:
		usberr("event %d (%d status)- not handled", event, state);
		break;
	}

	/* FIXME: BUG ! when open usbtty cannot use printf'debug -> printf' func */
	usbinfo("%s event: %d oldstate: %d newstate: %d status: %d address: %d\n",
