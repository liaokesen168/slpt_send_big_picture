...
static struct m200_udc_dev *udc_dev;
...
int jz4780_udc_init(void)
{
	...
	usbinfo("m200 usbd start\n");

	udc_dev = malloc(sizeof(struct m200_udc_dev));
	if (!udc_dev) {
		usbinfo("%s malloc memary error\n", udc_dev->name);
		return -EBUSY;
	}

	memset(udc_dev, 0, sizeof(struct m200_udc_dev));
