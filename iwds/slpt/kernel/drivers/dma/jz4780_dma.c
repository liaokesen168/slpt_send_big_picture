...
static struct {
	...
	jz4780_dma_chan_t chans[NR_DMA_CHANNELS];
...
static int jz4780_dma_init_done(void)
{
	return dmac->init_done;
}

jz4780_dma_chan_t *jz4780_dma_alloc_chan(jz4780_dma_filter_t filter)
{
	if (!jz4780_dma_init_done()) {
...
void jz4780_dma_release_chan(jz4780_dma_chan_t *chan)
{
	if (!jz4780_dma_init_done()) {
		printk(DRVNAME": WARNING! GPDMA controller is not available.\n");
		return;
...
void jz4780_dma_chan_dump(jz4780_dma_chan_t *chan)
{
