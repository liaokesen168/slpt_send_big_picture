...
static char *symbol_string(char *buf, char *end, unsigned long addr,
		int field_width, int precision, int flags)
{
	char sym[128];
	sprint_symbol(sym, addr);
	return string(buf, end, sym, field_width, precision,
			flags & ~SPECIAL);
}
		...
		int field_width, int precision, int flags)
{
	...
	switch (*fmt) {
	case 'S':
		return symbol_string(buf, end, (unsigned long)ptr, field_width,
				     precision, flags);
#ifdef CONFIG_CMD_NET
	case 'm':
		flags |= SPECIAL;
