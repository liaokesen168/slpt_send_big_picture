...
static struct date_en_view datev;

static void init_default_setting(void) {

}

int slpt_init_date_en_view(void) {
	struct slpt_app_res *clock_dir;

	clock_dir = slpt_kernel_name_to_app_res("clock", uboot_slpt_task);
	assert(clock_dir);

	assert(!init_date_en_view(&datev, "date-en-view"));

	assert(slpt_register_date_en(&datev, clock_dir));
