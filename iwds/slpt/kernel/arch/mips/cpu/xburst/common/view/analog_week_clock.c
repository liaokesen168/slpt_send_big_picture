...
static struct analog_week_clock clocka;

static void init_default_setting(void) {

}

int slpt_init_analog_week_clock(void) {
	struct slpt_app_res *clock_dir;

	clock_dir = slpt_kernel_name_to_app_res("clock", uboot_slpt_task);
	assert(clock_dir);

	assert(!init_analog_week_clock(&clocka, "analog-week-clock-viewa"));
	assert(slpt_register_analog_week_clock(&clocka, clock_dir));
