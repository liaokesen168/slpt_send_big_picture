...
#ifndef __MACH_M200_NAND_H__
#define __MACH_M200_NAND_H__
...
#define MAX_NAND_NAME_LENGTH    64
#define MAX_NUM_NAND_IF         7

#define VNAND_ECCPOS            4
#define VNAND_BADBLOCKPOS       0
#define VNAND_BADBLOCKBITS      64

/* SPL related parameters */
#define SPL_BAK_BLOCKS          2 /* number of SPL back-up blocks */
...
typedef struct {
	const char name[MAX_NAND_NAME_LENGTH];
	...
	int (*nand_pre_init)(struct m200_nand *nand);
} nand_flash_info_t;

struct ingenic_vnand {
	int	is_vnand;
	int eccpos;
	int badblockpos;
	int badblockbits;
...
struct m200_nand_platform_data {
	char part_default[1024];
...
extern int micron_nand_pre_init(struct m200_nand *nand);
extern int samsung_nand_pre_init(struct m200_nand *nand);
