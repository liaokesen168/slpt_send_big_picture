 ...
 * @mipi_dsi: using mipi dsi controller. 0: not use, 1: use
 * @dsi_pdata: mipi dsi platform data
...
struct jzfb_platform_data {
	struct {
		char *name;                  /* lcd device name (id) */
		int (*resource_init) (void); /* init lcd device resource */
		int (*power_on) (int on);	 /* power control : 0-off 1-on */
		int (*update_mode) (int update); /* update lcd mode, when entering/exiting u-boot */
	} lcd;
	...
	struct {
		...
		unsigned rdply_active_high:1;
		unsigned chip_select_valid_high:1;
		...
		int (*init)(void);
		/* 0, enable slcd dma restart_work_around; 1, disable slcd dma restart_work_around. */
		int disable_slcd_dma_restart_work_around;
		struct {
			int te_gpio;
			int te_gpio_level;
			unsigned int te_irq;
	...
	/* lcdc ops callback for board lcd operate*/
	struct jzfb *jzfb;
...
struct jzfb {
	...
	struct fb_struct fbs;    /* current fb buffer struct */
	struct fb_region region;    /* current fb region convert to from @fbs */
	int buffer_index;

	struct {
		unsigned int is_enabled:1;
		unsigned int is_inited:1;
		unsigned int is_panel_setted:1;
		unsigned int is_transfer:1;
		unsigned int is_gpio_transfer:1;
		unsigned int need_to_init_gpio:1;
		unsigned int need_to_save_base:1;
		unsigned int is_clk_enabled:1;
		unsigned int need_to_enable_fb:1;
		unsigned int need_to_enable_clk:1;
		unsigned int need_to_enable_irq:1;
	} s;

	int id;
	struct clk *clk;
	unsigned long base;
	struct jzfb_framedesc *fbd;
...
extern int jzfb_enable(struct jzfb *jzfb);
extern int jzfb_disable(struct jzfb *jzfb);
extern int jzfb_pan_dispaly(struct jzfb *jzfb, unsigned int offset);
extern void jzfb_lcdc_wait_dma_end(struct jzfb *jzfb);
extern int jzfb_init_data(struct jzfb *jzfb, struct jzfb_platform_data *pdata);
extern int jzfb_clk_enable(struct jzfb *jzfb);
extern int jzfb_clk_disable(struct jzfb *jzfb);
extern int jzfb_replace_kernel(struct jzfb* jzfb);
extern int jzfb_restore_kernel(struct jzfb* jzfb);
extern int jzfb_dma_transfer(struct jzfb *jzfb, unsigned int count);
extern int jzfb_flush_cache(struct jzfb *jzfb);
extern int jzfb_flush_fb(struct jzfb *jzfb, unsigned int offset);
extern void jzfb_set_transfer_mode(struct jzfb *jzfb, unsigned int mode);
extern int jzfb_update_status(struct jzfb *jzfb);
extern int jzfb_restore_status(struct jzfb *jzfb);
#endif
