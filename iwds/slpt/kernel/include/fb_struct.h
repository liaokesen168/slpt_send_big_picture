...
static const font_t DEFAULT_FONT = 0;
static const struct position DEFAULT_POSITION  = {0, 0};
...
static inline int font_equal(font_t f1,  font_t f2) {
	return f1 == f2;
}
...
extern int board_lcd_power_on(void);
extern int board_lcd_power_off(void);
extern int lcd_bm_init_data(void);

extern struct fb_struct* get_default_fb(void);
extern int lcd_pan_display(struct fb_struct *fbs, unsigned int offset);
extern void lcd_wait_display_end(struct fb_struct *fbs);
extern int lcd_flush_fb(struct fb_struct *fbs, unsigned int offset);
