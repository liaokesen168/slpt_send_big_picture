...
struct gpio_save {
/* save the kernel gpio reg state which is used in slpt */
	unsigned int pxint:1;
	unsigned int pxmsk:1;
	unsigned int pxpat1:1;
	unsigned int pxpat0:1;
	unsigned int pxpe:1;
};

#define CONFIG_MUTIPLE_I2C_BUS
...
struct client_i2c_bus {
	...
	int scl_gpio;
	int sda_gpio;
	struct gpio_save scl_in_kernel; /* save the kernel gpio state which is use in slpt for scl */
	struct gpio_save sda_in_kernel; /* save the kernel gpio state which is use in slpt for scl */
};

extern int mutiple_i2c_probe(uchar addr);
extern int mutiple_i2c_read(int bus_num, uchar chip, uint addr, int alen, uchar *buffer, int len);
extern int mutiple_i2c_write(int bus_num, uchar chip, uint addr, int alen, uchar *buffer, int len);
#endif
/*
...
static inline u8 mutiple_i2c_reg_read(struct client_i2c_bus *i2c_bus, u8 addr, u8 reg)
{
	...
	mutiple_i2c_read(i2c_bus->bus_num, addr, reg, 1, &buf, 1);
	return buf;
}
...
static inline void mutiple_i2c_reg_write(struct client_i2c_bus *i2c_bus, u8 addr, u8 reg, u8 val)
{
	...
	mutiple_i2c_write(i2c_bus->bus_num, addr, reg, 1, &val, 1);
}
#endif
